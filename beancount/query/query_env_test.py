...
class TestEnv(unittest.TestCase):
    ...
    def test_ToDecimal(self, entries, _, options_map):
        ...
        rtypes, rrows = query.run_query(entries, options_map,
                                        'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_stocks_us")) as m') 
        self.assertEqual([(D('0.6'),)], rrows)

        rtypes, rrows = query.run_query(entries, options_map,
                                        'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_bonds_us")) as m') 
        self.assertEqual([(None,)], rrows)

        rtypes, rrows = query.run_query(entries, options_map,
                                        'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_bonds_us"), 0.0) as m') 
        self.assertEqual([(D('0.0'),)], rrows)

        rtypes, rrows = query.run_query(entries, options_map,
                                        'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_stocks_us"))'
        ...
        with self.assertRaises(TypeError):
          rtypes, rrows = query.run_query(entries, options_map,
                                          'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_bonds_us"))'
                                          + '* number(convert(units(position), "USD")) as m')

        rtypes, rrows = query.run_query(entries, options_map,
                                        'SELECT decimal(getitem(commodity_meta(commodity(units(position))), "pcnt_bonds_us"), 0.0)'
                                        + '* number(convert(units(position), "USD")) as m')
